...
errors = (
    (0, "irta lmochkil we chkechm"),
    (1, "lvariable machi huwa hadak "),
    (2, "lvariable maybanch"),
    (3, "t3awed t79e9 men type"),
    (4, "irtla l3adad "),
    (5, "t3awed tbe9a hna"),
    (6, "ghir bchwiya machi 2 chkechm d9a we7da"),
    (7, "chi7aja ortli lmochkil")
)
...
class pattern:
    # general from : kism student -> int id ,
        ...
        if attributes == None:
            ...
            for par in list_attributes:
                ...
                if typee == "sahih":
                    ...
                    if value == "vre":
                        value = True
                self.count = len(list_attributes)
                if par.split(" ")[0] != '':
                    namee = par.split(" ")[1]
                else:
                    namee = par.split(" ")[2]
                attribute = variable(namee, typee, value)
...
def blockify(source):
    blocks = []
    b = [ "dalla ","ila ", "ma7d ", "kissm ", "t3awd "]
    s = ["ara", "chkechm", "dir", "sahih", "achari", "manti9i", "marka", "harf", "jme3lia", "ch7alkaynf"]
    i = 0
    j = 0
    while i < len(source):
        block = ""
        ...
        while j < len(source):
            ...
            if "t3awd" in key or "ila" in key or "ma7d" in key or "dalla" in key:
                o = 0
                e = 0
                for k in range(j+1, len(source)):
                    ...
                    if e == o and o != 0:
                        ...
                        break
            #Ordinary statements
            elif "ara"  in key or "chkechm" in key or "dir" in key or  "sahih" in key or "achari" in key or  "manti9i" in key or  "marka" in key or "harf" in key or "jme3lia" in key or "ch7alkaynf" in key:
                for k in range(j+1, len(source)):
                    if source[k] == ";":
                        block = source[i:k+1]
                        blocks.append(block)
